...
- **Concentrar ataques** (`AJUSTE_CONCENTRAR` + `ajusteAtaqueDirecionado`, 2026-08-07 — "Pelos lados" virou ataque DIRECIONADO): Pelo meio `{+1.4 meio}` (mais posse) · Pela esquerda/direita `{-0.5 meio, +0.9 atk}` + escanteios ×1.35 e mais chance de gol de escanteio (mesmo efeito que "lados" tinha antes) · Explorar brecha rival `{-0.3 meio, +0.6 atk}`, sem bônus de escanteio. As 3 direcionadas (esquerda/direita/brecha) somam um delta extra (±1.5, clamp) baseado no defensor INDIVIDUAL alvo em campo — não só a média do setor: "esquerda" mira o Lateral-Direito adversário, "direita" o Lateral-Esquerdo, "brecha" sempre o defensor mais fraco em campo (nunca negativo). Mirar um lado FORTE do adversário pune o atacante. Save antigo com `concentrar:"lados"` migra pra `"esquerda"` no load (preserva o bônus de escanteio que já tinha).
